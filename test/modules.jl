...
@testset "IdentityStateBundle Module" begin
    using ..IngExuity.IdentityStateBundle: create_bundle, bundle_to_dict, dict_to_bundle,
                apply_bundle, merge_bundle, get_identity_fingerprint, IdentityStateBundle
    using ..IngExuity.Types: SelfModel as SelfModelType, InternalEmotional as InternalEmotionalType,
                 Intelligence as IntelligenceType

    @testset "create_bundle creates IdentityStateBundle" begin
        sm = SelfModelType()
        ie = InternalEmotionalType()
        intel = IntelligenceType()

        bundle = create_bundle(sm, ie, intel, 42)
        @test bundle isa IdentityStateBundle
        @test bundle.identity == "IngEnuity"
        @test bundle.facts_stored == 42
    end

    @testset "bundle_to_dict returns serializable dict" begin
        sm = SelfModelType()
        ie = InternalEmotionalType()
        intel = IntelligenceType()

        bundle = create_bundle(sm, ie, intel, 10)
        d = bundle_to_dict(bundle)

        @test haskey(d, "identity")
        @test haskey(d, "emotional")
        @test haskey(d, "intelligence")
        @test d["identity"] == "IngEnuity"
    end

    @testset "dict_to_bundle reconstructs bundle" begin
        sm = SelfModelType()
        ie = InternalEmotionalType()
        intel = IntelligenceType()

        original = create_bundle(sm, ie, intel, 5)
        d = bundle_to_dict(original)
        reconstructed = dict_to_bundle(d)

        @test reconstructed.identity == original.identity
        @test reconstructed.confidence == original.confidence
    end

    @testset "apply_bundle updates self_model and internal" begin
        sm = SelfModelType()
        ie = InternalEmotionalType()
        intel = IntelligenceType()
        ie.valence = -0.5
        ...
        bundle = create_bundle(sm, ie, intel, 0)
        new_sm, new_ie = apply_bundle(bundle, sm, ie)

        @test new_ie.valence == -0.5
        @test new_ie.stress_level == 0.7
    end

    @testset "merge_bundle combines two bundles" begin
        sm = SelfModelType()
        ie1 = InternalEmotionalType()
        ie2 = InternalEmotionalType()
        ie2.valence = 0.5
        ie2.stress_level = 0.3
        intel = IntelligenceType()

        bundle1 = create_bundle(sm, ie1, intel, 5)
        bundle2 = create_bundle(sm, ie2, intel, 10)
        merged = merge_bundle(bundle1, bundle2)

        @test merged.facts_stored == 10
        @test merged.stress_level == 0.3
        @test merged.valence > 0.0
    end

    @testset "get_identity_fingerprint returns consistent hash" begin
        sm = SelfModelType()
        ie = InternalEmotionalType()
        intel = IntelligenceType()

        bundle = create_bundle(sm, ie, intel, 0)
        fp1 = get_identity_fingerprint(bundle)
        fp2 = get_identity_fingerprint(bundle)
...
@testset "ExecutionStateEntropy Module" begin
    using ..IngExuity.ExecutionStateEntropy: ExecutionEntropy, measure_entropy,
                compute_decision_variance, compute_prediction_uncertainty,
                compute_response_diversity, get_entropy_report
    using ..IngExuity.Types: InternalEmotional as InternalEmotionalType,
                 UserModel as UserModelType, Prediction
    using ..IngExuity.HumanInput: process
    using Dates

    @testset "ExecutionEntropy initializes correctly" begin
        entropy = ExecutionEntropy()
        @test entropy.decision_variance == 0.0
        @test entropy.prediction_uncertainty == 0.0
        @test entropy.entropy_score == 0.0
        @test entropy.stability_label == "stable"
    end

    @testset "measure_entropy updates all metrics" begin
        entropy = ExecutionEntropy()
        ie = InternalEmotionalType()
        ie.valence = 0.3
        ie.arousal = 0.6
        ...
        preds = [
            Prediction("action1", "need1", 0.7, [:test], now()),
            Prediction("action2", "need2", 0.8, [:test], now())
        ]

        updated = measure_entropy(entropy, ie, preds, 50)

        @test updated.entropy_score >= 0.0
        @test updated.entropy_score <= 1.0
    end

    @testset "compute_decision_variance" begin
        decisions = [0.5, 0.6, 0.7, 0.5, 0.6]
        variance = compute_decision_variance(decisions)
        @test variance >= 0.0
    end

    @testset "compute_prediction_uncertainty with empty predictions" begin
        uncertainty = compute_prediction_uncertainty(Prediction[])
        @test uncertainty == 1.0
    end

    @testset "get_entropy_report returns full report" begin
        entropy = ExecutionEntropy()
        report = get_entropy_report(entropy)

        @test haskey(report, "entropy_score")
        @test haskey(report, "stability_label")
        @test haskey(report, "decision_variance")
        @test haskey(report, "prediction_uncertainty")
        @test haskey(report, "response_diversity")
    end
end

@testset "InstanceCommunication Module" begin
    using ..IngExuity.InstanceCommunication: InstanceMessage, InstanceRegistry,
                register_instance, unregister_instance, send_message,
                broadcast_to_all, sync_state, get_active_instances

    @testset "InstanceRegistry initializes" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        @test registry isa InstanceRegistry
        @test isempty(registry.instances)
    end

    @testset "unregister_instance removes from registry" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        register_instance(registry, "instance-1")
        ok = unregister_instance(registry, "instance-1")
        @test ok == true
        @test !haskey(registry.instances, "instance-1")
    end

    @testset "send_message queues message to registered instance" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        register_instance(registry, "to-1")
        msg = InstanceMessage("from-1", "to-1", :ping, Dict{String,Any}("data" => "test"))
        ok = send_message(registry, msg)
        @test ok == true
        @test length(registry.message_queue) == 1
    end

    @testset "send_message returns false for unknown instance" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        msg = InstanceMessage("from-1", "unknown", :ping, Dict{String,Any}("data" => "test"))
        ok = send_message(registry, msg)
        @test ok == false
    end

    @testset "broadcast_to_all sends to all other instances" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        register_instance(registry, "instance-1")
        register_instance(registry, "instance-2")
        register_instance(registry, "instance-3")

        count = broadcast_to_all(registry, "instance-1", :state_sync, Dict{String,Any}("turn" => 1))
        @test count == 2
    end

    @testset "sync_state updates instance state" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        register_instance(registry, "instance-1")
        ok = sync_state(registry, "instance-1", Dict{String,Any}("turn_count" => 5))
        @test ok == true
        @test registry.instances["instance-1"]["state"]["turn_count"] == 5
    end

    @testset "get_active_instances returns all instances" begin
        registry = InstanceRegistry("test-instance", Dict{String,Dict{String,Any}}(), InstanceMessage[], false)
        register_instance(registry, "instance-1")
        register_instance(registry, "instance-2")

        instances = get_active_instances(registry)
        @test length(instances) == 2
    end
end

@testset "MobileWASM Module" begin
    using ..IngExuity.MobileWASM: WASMConfig, get_wasm_config, is_wasm_build,
                create_wasm_init_script, setup_wasm_exports

    @testset "WASMConfig initializes with defaults" begin
        config = WASMConfig()
        @test config.heap_size == 512 * 1024 * 1024
        @test config.stack_size == 8 * 1024 * 1024
        @test config.enable_gc == true
        @test config.enable_simd == true
        @test config.optimization_level == 3
    end

    @testset "get_wasm_config returns config" begin
        config = get_wasm_config()
        @test config isa WASMConfig
    end

    @testset "is_wasm_build checks environment" begin
        build_flag = is_wasm_build()
        @test build_flag isa Bool
    end

    @testset "create_wasm_init_script generates script" begin
        config = WASMConfig()
        script = create_wasm_init_script("IngExuity", config)
        @test occursin("IngExuity WASM", script)
        @test occursin("WASM_CONFIG", script)
    end

    @testset "setup_wasm_exports returns export functions" begin
        exports = setup_wasm_exports()
        @test haskey(exports, "ingexuity_init")
        @test haskey(exports, "ingexuity_chat")
        @test haskey(exports, "ingexuity_reset")
    end
end
...
@testset "Integration: Full conversation flow" begin
    ...
    @testset "stress detection triggers stay_present" begin
        ...
        input = process("I'm so stressed and overwhelmed")
        comp = comprehend(input)
